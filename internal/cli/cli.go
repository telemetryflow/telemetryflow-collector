// Package cli provides CLI command creation for TelemetryFlow Collector.
//
...
Use --otel flag to enable full OTEL capabilities including:
  ...
  - Full OpenTelemetry ecosystem compatibility`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if opts.UseOTEL {
				return RunOTELCollector(opts)
			}
			return RunCollector(opts)
		},
	}

	cmd.Flags().BoolVar(&opts.UseOTEL, "otel", false, "Use full OTEL collector with all capabilities (metrics, logs, traces, exemplars)")
	return cmd
}
...
func NewConfigCmd(opts *Options) *cobra.Command {
	...
	validateCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cfg, err := loader.Load(opts.CfgFile)
			if err != nil {
...
// NewValidateCmd returns the validate command (top-level shortcut).
func NewValidateCmd(opts *Options) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			loader := config.NewLoader()
			_, err := loader.Load(opts.CfgFile)
			if err != nil {
				return fmt.Errorf("configuration validation failed: %w", err)
			}
...
func RunCollector(opts *Options) error {
	// Load configuration
	loader := config.NewLoader()
	cfg, err := loader.Load(opts.CfgFile)
	if err != nil {
...
func RunOTELCollector(opts *Options) error {
	// Determine config path
	configPath := opts.CfgFile
	if configPath == "" {
